...
const walletSchema = new Schema({ organization: { type: Schema.Types.ObjectId, ref: "Organization", required: true }, balance: { ...money, default: 0 }, currency: { type: String, default: "NGN" }, status: { type: String, enum: ["ACTIVE", "FROZEN"], default: "ACTIVE" } }, timestamps);
walletSchema.index({ organization: 1 }, { unique: true });
const ledgerSchema = new Schema({ organization: oid("Organization", true), type: { type: String, enum: ["CREDIT", "DEBIT"], required: true }, amount: { ...money, required: true, min: 0.01 }, balanceAfter: { ...money, required: true }, reference: { type: String, required: true, unique: true }, payment: oid("OrganizationPayment"), narration: String, createdBy: oid("User") }, timestamps);
ledgerSchema.index({ organization: 1, createdAt: -1 });
const withdrawalSchema = new Schema({ organization: oid("Organization", true), requestedBy: oid("User", true), amount: money, status: { type: String, default: "UNAVAILABLE" }, reason: { type: String, default: "Bank settlement is not enabled for organizations." } }, timestamps);
...
const models = {
  ...
  OrganizationWithdrawal: [withdrawalSchema, "OrganizationWithdrawal"], OrganizationAnnouncement: [announcementSchema, "OrganizationAnnouncement"],
