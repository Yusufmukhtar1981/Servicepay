  ...
  try {
    ...
    if (!senderId) {
      return res.status(401).json({
        success: false,
        message: "Please log in before making a transfer.",
      });
    }
    ...
    if (receiver.status !== "ACTIVE") {
      ...
      return res.status(403).json({
        success: false,
        message:
          "The receiver's account is not active.",
      });
    }
    ...
    if (!updatedSender) {
      ...
      return res.status(400).json({
        success: false,
        message:
          "Insufficient wallet balance or debit failed.",
      });
    }
    ...
    if (!updatedReceiver) {
      throw new Error(
        "Unable to credit the receiver's wallet."
      );
    }
