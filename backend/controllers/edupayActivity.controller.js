...
const parentRecordDto = (row) => {
  ...
  const allowed = { ATTENDANCE: ["status", "note", "date", "eventDate", "class", "session", "term"], RESULT: ["subject", "ca", "exam", "total", "grade", "position", "teacherComment"], ASSIGNMENT: ["title", "subject", "description", "dueDate", "status"], ACTIVITY: ["title", "description", "category", "image", "date"], CONDUCT: ["category", "note", "severity", "title"], ANNOUNCEMENT: ["title", "description", "message", "date", "eventDate"] }[row.recordType] || [];
  const clean = Object.fromEntries(allowed.filter((key) => payload[key] !== undefined).map((key) => [key, payload[key]]));
  return { id: row._id, type: row.recordType, eventDate: row.eventDate, date: payload.date || row.eventDate, submittedAt: row.createdAt || null, updatedAt: row.updatedAt || null, status: row.status, audience: row.audience, payload: clean };
};
