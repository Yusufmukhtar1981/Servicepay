...
/*
 * Referral codes are public identifiers. Keep their accepted shape narrow so
 * query strings cannot be used to smuggle arbitrary values into lookups.
 * Codes generated by older releases are still accepted when they already
 * follow the same simple, case-insensitive format.
 */
...
const buildReferralCode = (fullName) => {
  const rawName = String(fullName || "USER")
    .replace(/[^A-Za-z0-9]/g, "")
    .toUpperCase();
  const prefix = rawName.substring(0, 4).padEnd(4, "X");
  const random = crypto.randomBytes(5).toString("hex").toUpperCase();

  return `SP-${prefix}-${random}`;
};

const findEligibleReferrer = async (value) => {
  ...
  const settings = await AppSettings.findOne({
    ...
    .select("registration.allowReferralCode")
    .lean();

  if (settings?.registration?.allowReferralCode === false) {
    return null;
  }
  ...
  return User.findOne({
    ...
    status: "ACTIVE",
    isDeleted: { $ne: true },
    activationPending: { $ne: true },
  })
    .select("_id fullName referralCode")
    .lean();
};

const isReferralCodeCollision = (error) => {
  if (error?.code !== 11000) {
    return false;
  }

  if (error?.keyPattern?.referralCode || error?.keyValue?.referralCode) {
    return true;
  }
...
const createCustomerWithUniqueReferralCode = async (
  ...
  for (let attempt = 0; attempt < 10; attempt += 1) {
    ...
    try {
       ...
       * registrations. Retry only a referral-code collision; phone/email
...
exports.registerUser = async (
  ...
  try {
    ...
    /*
     * Referral attribution is deliberately resolved before User.create but
     * is written only as part of that create. Invalid or ineligible codes are
     * ignored so they can never prevent an otherwise valid registration.
     */
    const eligibleReferrer = await findEligibleReferrer(referralCode);
    const referralAttribution = eligibleReferrer
      ? {
          referredBy: eligibleReferrer._id,
  ...
  try {
    ...
    if (!user.referralCode) {
      ...
      for (
        ...
        const candidate = buildReferralCode(user.fullName);
...
/*
 * Public referral-code validation. This endpoint intentionally discloses no
 * account details beyond a referrer's first name after all eligibility
 * checks pass.
 */
exports.validateReferralCode = async (req, res) => {
  try {
    const referrer = await findEligibleReferrer(req.query?.code);
    ...
    return res.status(200).json({
      success: true,
      valid: true,
      firstName: firstNameFromFullName(referrer.fullName),
    });
  } catch (error) {
    console.error("VALIDATE REFERRAL ERROR:", error);

    /*
     * Referral validation is advisory. A datastore issue must not turn a
     * missing/invalid referral into a registration failure, but the public
     * endpoint still reports its failure explicitly.
     */
    return res.status(500).json({
      success: false,
      valid: false,
