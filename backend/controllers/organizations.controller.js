...
exports.safeSettlementAccount = safeSettlementAccount;
...
exports.adminWithdrawalDetail = async (req, res) => { if (!adminScope(req)) return fail(res, 403, "Platform administrator access is required."); const withdrawal = await models.OrganizationWithdrawal.findById(req.params.id).populate("organization", "name code").lean(); return withdrawal ? ok(res, { data: withdrawal, withdrawal, events: await models.OrganizationWithdrawalSnapshot.find({ withdrawal: withdrawal._id }).sort({ createdAt: 1 }).lean() }) : fail(res, 404, "Withdrawal not found."); };
exports.adminReviewWithdrawal = async (req, res) => { if (!adminScope(req)) return fail(res, 403, "Platform administrator access is required."); try { const withdrawal = await treasury.transition(req, req.params.id, String(req.body.action || "").toUpperCase() === "REJECT" ? "REJECTED" : "APPROVED", true); const result = withdrawal.status === "APPROVED" ? await treasury.dispatch(withdrawal._id) : { withdrawal, configurationRequired: false }; return ok(res, { data: result.withdrawal, withdrawal: result.withdrawal, configurationRequired: result.configurationRequired }); } catch (e) { return fail(res, e.status || 500, e.message); } };
exports.adminSettlementAccounts = async (req, res) => { if (!adminScope(req)) return fail(res, 403, "Platform administrator access is required."); const accounts = await models.OrganizationSettlementAccount.find({ ...(req.query.organizationId && id(req.query.organizationId) ? { organization: req.query.organizationId } : {}), ...(req.query.status ? { status: String(req.query.status).toUpperCase() } : {}) }).select("-accountNumber").sort({ createdAt: -1 }).limit(200).populate("organization", "name code").lean(); return ok(res, { data: accounts, accounts }); };
exports.adminReviewSettlementAccount = async (req, res) => { if (!adminScope(req)) return fail(res, 403, "Platform administrator access is required."); const status = String(req.body.status || "").toUpperCase(); if (!["VERIFIED", "REJECTED", "DISABLED"].includes(status)) return fail(res, 400, "Invalid settlement account status."); const coolingHours = Math.min(720, Math.max(0, Number(process.env.ORG_SETTLEMENT_COOLING_HOURS || 24))); const account = await models.OrganizationSettlementAccount.findByIdAndUpdate(req.params.id, { $set: { status, reviewedBy: req.user._id, reviewedAt: new Date(), rejectionReason: String(req.body.reason || "").slice(0, 500), ...(status === "VERIFIED" ? { coolingOffUntil: new Date(Date.now() + coolingHours * 3600000) } : {}) } }, { new: true }).select("-accountNumber"); if (!account) return fail(res, 404, "Settlement account not found."); await svc.audit(req, { _id: account.organization }, `SETTLEMENT_ACCOUNT_${status}`, "OrganizationSettlementAccount", account._id, { status, coolingHours }); return ok(res, { data: account, account }); };
exports.adminTreasuryConfig = async (req, res) => {
  ...
  if (!id(organizationId)) return fail(res, 400, "Organization id is required.");
  if (req.method === "GET") {
    return ok(res, {
      data: await models.OrganizationTreasuryConfig.findOne({ organization: organizationId }).lean(),
    });
  }

  const fields = ["authorizationMode", "minimumWithdrawal", "maximumWithdrawal", "dailyLimit", "monthlyLimit", "fee"];
  const update = {};
  try {
    for (const field of fields) {
      if (req.body[field] !== undefined) {
        update[field] = field === "authorizationMode"
          ? String(req.body[field]).toUpperCase()
          : svc.normalizeMoney(req.body[field]);
      }
    }
    const current = await models.OrganizationTreasuryConfig.findOne({ organization: organizationId }).lean();
    const candidate = {
    ...
    if (candidate.minimumWithdrawal > candidate.maximumWithdrawal) {
      return fail(res, 400, "Minimum withdrawal cannot exceed maximum withdrawal.");
    }
    if (
      candidate.maximumWithdrawal > candidate.dailyLimit ||
      candidate.maximumWithdrawal > candidate.monthlyLimit ||
      candidate.dailyLimit > candidate.monthlyLimit
    ) {
      return fail(res, 400, "Withdrawal limits must satisfy transaction maximum ≤ daily limit ≤ monthly limit.");
    }
  } catch (e) {
    return fail(res, 400, e.message);
  }

  const config = await models.OrganizationTreasuryConfig.findOneAndUpdate(
    { organization: organizationId },
    { $set: { ...update, updatedBy: req.user._id } },
    { upsert: true, new: true, runValidators: true },
  );
  await svc.audit(req, { _id: organizationId }, "TREASURY_CONFIG_UPDATED", "OrganizationTreasuryConfig", config._id, { fields: Object.keys(update) });
  return ok(res, { data: config, config });
};
...
exports.withdraw = async (req, res) => {
  try { const org = await requireOrg(req, "wallet.withdraw"); if (!org) return fail(res, 403, "Organization access denied."); const result = await treasury.createWithdrawal(req, org); const dispatched = !result.duplicate && result.withdrawal.status === "APPROVED" ? await treasury.dispatch(result.withdrawal._id) : { withdrawal: result.withdrawal, configurationRequired: false }; const configurationRequired = Boolean(dispatched.configurationRequired); return res.status(result.duplicate ? 200 : 201).json({ success: true, duplicate: result.duplicate, configurationRequired, data: dispatched.withdrawal, withdrawal: dispatched.withdrawal }); }
  catch (e) { return fail(res, e.status || e.statusCode || 500, e.message); }
};
exports.treasury = async (req, res) => {
  const org = await requireOrg(req, "wallet.view"); if (!org) return fail(res, 403, "Organization access denied.");
  const [wallet, withdrawals] = await Promise.all([OrganizationWallet.findOne({ organization: org._id }).lean(), models.OrganizationWithdrawal.find({ organization: org._id, status: { $in: ["INITIATED", "PENDING_APPROVAL", "APPROVED", "PROCESSING", "PENDING_REVIEW"] } }).sort({ createdAt: -1 }).lean()]);
  const w = wallet || { balance: 0, heldBalance: 0 }; return ok(res, { data: { ledgerBalance: w.balance || 0, heldBalance: w.heldBalance || 0, availableBalance: Math.max(0, (w.balance || 0) - (w.heldBalance || 0)), pendingWithdrawals: withdrawals.reduce((n, x) => n + Number(x.totalDebit || 0), 0), totalMoneyIn: w.totalMoneyIn || 0, totalWithdrawn: w.totalWithdrawn || 0, totalFees: w.totalFees || 0, recentTransactions: await OrganizationLedger.find({ organization: org._id }).sort({ createdAt: -1 }).limit(25).lean() }, wallet: w, withdrawals });
};
exports.settlementAccounts = async (req, res) => {
  try { const org = await requireOrg(req, req.method === "POST" ? "treasury.accounts" : "wallet.view"); if (!org) return fail(res, 403, "Organization access denied."); if (req.method === "GET") return ok(res, { data: await models.OrganizationSettlementAccount.find({ organization: org._id }).select("-accountNumber").sort({ createdAt: -1 }).lean(), accounts: await models.OrganizationSettlementAccount.find({ organization: org._id }).select("-accountNumber").sort({ createdAt: -1 }).lean() }); const bankCode = String(req.body.bankCode || "").trim(); const accountNumber = String(req.body.accountNumber || "").replace(/\D/g, ""); if (!/^\d{10}$/.test(accountNumber) || !bankCode) return fail(res, 400, "Valid bank and 10-digit account number are required."); const resolved = await treasury.resolveAccount({ bankCode, accountNumber }); const account = await models.OrganizationSettlementAccount.create({ organization: org._id, bankCode, bankName: String(req.body.bankName || bankCode).slice(0, 120), accountNumber, accountNumberLast4: accountNumber.slice(-4), accountName: resolved.accountName, accountType: String(req.body.accountType || "").slice(0, 50), primary: req.body.primary === true, status: "PENDING" }); await svc.audit(req, org, "SETTLEMENT_ACCOUNT_ADDED", "OrganizationSettlementAccount", account._id, { bankCode, accountNumberLast4: account.accountNumberLast4 }); const safe = safeSettlementAccount(account); return res.status(201).json({ success: true, data: safe, account: safe }); }
  catch (e) { return fail(res, e.status || 500, e.message); }
};
exports.resolveSettlementAccount = async (req, res) => { try { const bankCode = String(req.body.bankCode || "").trim(); const accountNumber = String(req.body.accountNumber || "").replace(/\D/g, ""); if (!/^\d{10}$/.test(accountNumber) || !bankCode) return fail(res, 400, "Valid bank and 10-digit account number are required."); const resolved = await treasury.resolveAccount({ bankCode, accountNumber }); return ok(res, { data: { accountName: resolved.accountName, accountNumberLast4: accountNumber.slice(-4), accountNumberMasked: `******${accountNumber.slice(-4)}` } }); } catch (e) { return fail(res, e.status || 500, e.message); } };
exports.withdrawalList = async (req, res) => { const org = await requireOrg(req, "wallet.view"); if (!org) return fail(res, 403, "Organization access denied."); return ok(res, { data: await models.OrganizationWithdrawal.find({ organization: org._id }).sort({ createdAt: -1 }).limit(100).lean(), withdrawals: await models.OrganizationWithdrawal.find({ organization: org._id }).sort({ createdAt: -1 }).limit(100).lean() }); };
exports.withdrawalDetail = async (req, res) => { const org = await requireOrg(req, "wallet.view"); if (!org) return fail(res, 403, "Organization access denied."); const withdrawal = await models.OrganizationWithdrawal.findOne({ _id: req.params.withdrawalId, organization: org._id }).lean(); return withdrawal ? ok(res, { data: withdrawal, withdrawal, events: await models.OrganizationWithdrawalSnapshot.find({ withdrawal: withdrawal._id }).sort({ createdAt: 1 }).lean() }) : fail(res, 404, "Withdrawal not found."); };
exports.approveWithdrawal = async (req, res) => { try { const org = await requireOrg(req, "treasury.approve"); if (!org) return fail(res, 403, "Organization access denied."); const approved = await treasury.transition(req, req.params.withdrawalId, "APPROVED"); const result = approved.status === "APPROVED" ? await treasury.dispatch(approved._id) : { withdrawal: approved, configurationRequired: false }; return ok(res, { data: result.withdrawal, withdrawal: result.withdrawal, configurationRequired: result.configurationRequired }); } catch (e) { return fail(res, e.status || 500, e.message); } };
exports.rejectWithdrawal = async (req, res) => { try { const org = await requireOrg(req, "treasury.approve"); if (!org) return fail(res, 403, "Organization access denied."); const withdrawal = await treasury.transition(req, req.params.withdrawalId, "REJECTED"); return ok(res, { data: withdrawal, withdrawal }); } catch (e) { return fail(res, e.status || 500, e.message); } };
exports.requeryWithdrawal = async (req, res) => { try { const org = await requireOrg(req, "wallet.view"); if (!org) return fail(res, 403, "Organization access denied."); const withdrawal = await models.OrganizationWithdrawal.findOne({ _id: req.params.withdrawalId, organization: org._id }); if (!withdrawal) return fail(res, 404, "Withdrawal not found."); const result = await treasury.requery(withdrawal._id); return ok(res, { data: result.withdrawal, withdrawal: result.withdrawal, ambiguous: result.ambiguous, alreadyRunning: result.alreadyRunning }); } catch (e) { return fail(res, e.status || 500, e.message); } };
