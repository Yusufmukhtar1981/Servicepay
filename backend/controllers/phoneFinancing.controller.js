...
async function debit(req, app, amount, type, session, finance = null, allocation = null) {
 ...
 const payer=await User.findById(uid(req)).select("walletBalance").session(session);if(!payer)throw error("Customer account not found.",401);await verifyTransactionPin(payer._id,req.body?.transactionPin ?? req.body?.pin,{session});
