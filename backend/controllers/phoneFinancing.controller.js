...
const officerBranchFilter = req =>
  req.staffAccess?.scope?.type === "BRANCH"
...
exports.submit = async (req, res) => {
 try {
  ...
  const application = await Application.create({ reference: ref("SPF-PHONE"), customer: uid(req), branchId: user.branchId || null, product: product._id, productSnapshot: productSnapshot(product), kycSnapshot, profileSnapshot,applicationInput:input, statusHistory: [{ status: "SUBMITTED", changedBy: uid(req), note: "Application submitted" }] });
...
exports.officerApplications = async (req,res) => {
  const filter={assignedOfficer:uid(req),assignmentState:"ACTIVE",...officerBranchFilter(req)}; if(req.params.applicationId) filter._id=req.params.applicationId; if(req.query.status) filter.status=text(req.query.status,40).toUpperCase();
...
exports.officerVerification = async (req,res) => { try {
  const app=await Application.findOne({...assignmentQuery(req.params.applicationId,uid(req)),...officerBranchFilter(req)});if(!app)throw error("Assigned phone application not found.",404);
...
exports.pay=async(req,res)=>{
  ...
  }catch(e){res.status(e.statusCode||500).json({success:false,code:e.code,message:e.message});}
