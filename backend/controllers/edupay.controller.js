...
const { getSettings, audit, notify, round, reference, hash, ensureObjectId, calculateSettlement, availableSavings, contributeFromWallet, repayFromWallet, createEduLedger, models } = require("../services/edupay.service");
...
exports.sponsorView = async (req, res) => {
  try { const invite = await EduPaySponsorInvite.findOne({ tokenHash: hash(req.params.token), status: "ACTIVE", expiresAt: { $gt: new Date() } }).populate("child plan"); if (!invite) return res.status(404).json({ success: false, message: "Sponsor link is invalid or expired." }); res.json({ success: true, sponsor: { childName: invite.child.fullName, school: invite.plan.school, expiresAt: invite.expiresAt, planId: invite.plan._id } }); } catch (error) { errorResponse(res, error); }
};
exports.sponsorContribute = async (req, res) => {
  try {
    const amount = round(req.body.amount); if (!(amount > 0) || !requireKey(req)) return res.status(400).json({ success: false, message: "A valid amount and Idempotency-Key are required." });
    const invite = await EduPaySponsorInvite.findOne({ tokenHash: hash(req.params.token), status: "ACTIVE", expiresAt: { $gt: new Date() } });
    if (!invite) return res.status(404).json({ success: false, message: "Sponsor link is invalid or expired." });
    const existing = await EduPaySponsorContribution.findOne({ idempotencyKey: requireKey(req) }); if (existing) return res.json({ success: true, duplicate: true, contribution: existing });
    const session = await mongoose.startSession(); let contribution;
    try { await session.withTransaction(async () => { const [row] = await EduPaySponsorContribution.create([{ invite: invite._id, parent: invite.parent, child: invite.child, plan: invite.plan, sponsorName: req.body.sponsorName || invite.sponsorName, amount, reference: reference("EDU-SPN"), idempotencyKey: requireKey(req) }], { session }); const saved = await availableSavings(invite.plan, session); await createEduLedger({ parent: invite.parent, child: invite.child, plan: invite.plan, direction: "CREDIT", type: "SPONSOR_CONTRIBUTION", amount, openingBalance: saved, reference: `${row.reference}-SAVINGS`, idempotencyKey: `${row.idempotencyKey}-savings`, source: "SPONSOR", session }); contribution = row; }); } finally { await session.endSession(); }
    await notify(invite.parent, "Sponsor contribution received", "A sponsor contributed to your EduPay plan.", contribution._id); res.status(201).json({ success: true, contribution });
  } catch (error) { errorResponse(res, error); }
};
...
exports.applySchool = async (req, res) => { try { const school = await School.create(req.body); res.status(201).json({ success: true, school }); } catch (error) { errorResponse(res, error); } };
...
exports.schoolCreateSession = async (req, res) => { try { const row = await EduPayAcademicSession.create({ school: req.eduPaySchool._id, name: req.body.name, startsAt: req.body.startsAt, endsAt: req.body.endsAt, status: "DRAFT" }); res.status(201).json({ success: true, session: row }); } catch (error) { errorResponse(res, error); } };
exports.schoolCreateTerm = async (req, res) => { try { const row = await EduPayTerm.create({ school: req.eduPaySchool._id, session: req.body.session, name: req.body.name, startsAt: req.body.startsAt, endsAt: req.body.endsAt, status: "DRAFT" }); res.status(201).json({ success: true, term: row }); } catch (error) { errorResponse(res, error); } };
exports.schoolCreateClass = async (req, res) => { try { const row = await EduPayClass.create({ school: req.eduPaySchool._id, name: req.body.name }); res.status(201).json({ success: true, classLevel: row }); } catch (error) { errorResponse(res, error); } };
exports.schoolCreateFee = async (req, res) => { try { const row = await Fee.create({ school: req.eduPaySchool._id, session: req.body.session, term: req.body.term, classLevel: req.body.classLevel, amount: round(req.body.amount), submittedBy: req.user._id, status: "PENDING_APPROVAL" }); res.status(201).json({ success: true, fee: row }); } catch (error) { errorResponse(res, error); } };
...
exports.adminCreateSettlement = async (req, res) => { try { const settings = await getSettings(); const plan = await Plan.findById(req.params.planId); if (!plan) return res.status(404).json({ success: false, message: "Plan not found." }); if (await Settlement.findOne({ plan: plan._id })) return res.status(409).json({ success: false, message: "Settlement already exists for this plan." }); const saved = await availableSavings(plan._id); const snapshot = calculateSettlement({ officialFee: plan.officialFee, saved, settings, settlementDate: req.body.settlementDate ? new Date(req.body.settlementDate) : new Date() }); const settlement = await Settlement.create({ ...snapshot, parent: plan.parent, child: plan.child, school: plan.school, plan: plan._id, reference: reference("EDU-SET"), idempotencyKey: requireKey(req) || reference("EDU-CMD"), status: "ADMIN_REVIEW" }); await Plan.updateOne({ _id: plan._id }, { $set: { status: "ADMIN_REVIEW" } }); await audit({ actor: req.user._id, action: "EDUPAY_SETTLEMENT_CREATED", entityType: "EduPaySettlement", entityId: settlement._id, school: settlement.school, req }); res.status(201).json({ success: true, settlement }); } catch (error) { errorResponse(res, error); } };
exports.adminSettlementAction = async (req, res) => { try { const settlement = await Settlement.findById(req.params.settlementId); if (!settlement) return res.status(404).json({ success: false, message: "Settlement not found." }); const action = String(req.body.action || "").toUpperCase(); if (action === "APPROVE") { if (settlement.status !== "ADMIN_REVIEW") return res.status(409).json({ success: false, message: "Settlement is not awaiting approval." }); settlement.status = "APPROVED"; settlement.approvedBy = req.user._id; settlement.approvedAt = new Date(); } else if (action === "PROCESS") { if (!["APPROVED", "PROCESSING"].includes(settlement.status)) return res.status(409).json({ success: false, message: "Settlement must be approved first." }); settlement.status = "PROCESSING"; } else if (action === "CONFIRM") { if (settlement.status === "SETTLED") return res.json({ success: true, duplicate: true, settlement }); if (settlement.status !== "PROCESSING" || !req.body.providerReference || !requireKey(req)) return res.status(400).json({ success: false, message: "Processing status, providerReference, and Idempotency-Key are required." }); settlement.status = "SETTLED"; settlement.providerReference = req.body.providerReference; settlement.provider = req.body.provider || "EXTERNAL_CONFIRMED"; settlement.confirmedBy = req.user._id; settlement.confirmedAt = new Date(); const repayment = await EduPayRepayment.findOne({ settlement: settlement._id }); if (!repayment && settlement.servicepayFundedPrincipal > 0) await EduPayRepayment.create({ parent: settlement.parent, child: settlement.child, plan: settlement.plan, settlement: settlement._id, principal: settlement.servicepayFundedPrincipal, serviceCharge: settlement.parentChargeAmount, totalAmount: settlement.parentTotalRepayment, amountPaid: 0, amountRemaining: settlement.parentTotalRepayment, dueDate: new Date(Date.now() + (await getSettings()).defaultRepaymentPeriodDays * 86400000), status: "ACTIVE" }); if (settlement.parentSavedAmount > 0) await createEduLedger({ parent: settlement.parent, child: settlement.child, plan: settlement.plan, direction: "DEBIT", type: "SETTLEMENT_DEBIT", amount: settlement.parentSavedAmount, openingBalance: await availableSavings(settlement.plan), reference: `${settlement.reference}-SAVINGS-DEBIT`, idempotencyKey: `${settlement.idempotencyKey}-savings-debit`, source: "SETTLEMENT" }); await Plan.updateOne({ _id: settlement.plan }, { $set: { status: "SETTLED" } }); } else if (action === "FAIL") { settlement.status = "FAILED"; settlement.failureReason = req.body.reason || "Settlement failed"; } else if (action === "REVERSE") { if (settlement.status !== "SETTLED") return res.status(409).json({ success: false, message: "Only settled records may be reversed." }); const Reversal = require("../models/edupaySettlementReversal.model"); const reversal = await Reversal.create({ settlement: settlement._id, plan: settlement.plan, parent: settlement.parent, school: settlement.school, amount: settlement.schoolGrossSettlement, reference: reference("EDU-REV"), reason: req.body.reason || "Settlement reversed", actor: req.user._id }); await Plan.updateOne({ _id: settlement.plan }, { $set: { status: "REVERSED" } }); await audit({ actor: req.user._id, action: "EDUPAY_SETTLEMENT_REVERSED", entityType: "EduPaySettlementReversal", entityId: reversal._id, school: settlement.school, req }); return res.json({ success: true, reversal, settlement }); } else return res.status(400).json({ success: false, message: "Unsupported settlement action." }); await settlement.save(); await audit({ actor: req.user._id, action: `EDUPAY_SETTLEMENT_${action}`, entityType: "EduPaySettlement", entityId: settlement._id, school: settlement.school, metadata: { providerReference: req.body.providerReference }, req }); res.json({ success: true, settlement }); } catch (error) { errorResponse(res, error); } };
