...
const { getSettings, audit, notify, round, reference, hash, ensureObjectId, calculateSettlement, availableSavings, contributeFromWallet, contributeSponsorFromWallet, repayFromWallet, confirmSettlement, reverseSettlement, createEduLedger, models } = require("../services/edupay.service");
...
exports.adminSettlementActionSecure = async (req, res) => {
  try {
    ...
    if (action === "CONFIRM") {
      const result = await confirmSettlement({ settlementId: settlement._id, actor: req.user._id, transactionId: req.body.transactionId, providerReference: req.body.providerReference, idempotencyKey: requireKey(req), req });
      return res.json({ success: true, duplicate: result.duplicate, settlement: result.settlement });
    }
    if (action === "REVERSE") {
