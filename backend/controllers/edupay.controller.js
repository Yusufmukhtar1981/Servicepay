...
exports.adminReadiness = async (req, res) => { try { const modelEntries = Object.entries(models); await EduPayDutyAssignment.init(); await Promise.all(modelEntries.filter(([, model]) => model?.init).map(([, model]) => model.init())); const indexes = {}; await Promise.all(modelEntries.filter(([, model]) => model?.collection?.listIndexes).map(async ([name, model]) => { indexes[name] = await model.collection.listIndexes().toArray(); })); const [manage, verify, process] = await Promise.all(["account.manage", "account.verify", "settlement.process"].map((permission) => EduPayDutyAssignment.countDocuments({ active: true, permissions: permission }))); const dutyCoverage = { manage, verify, process, ready: manage > 0 && verify > 0 && process > 0 }; res.json({ success: true, ready: dutyCoverage.ready, dutyCoverage, checkedAt: new Date(), indexes }); } catch (error) { errorResponse(res, error); } };
...
exports.adminVerifySettlementAccount = async (req, res) => { try { const result = await edupaySquad.verifyAccount({ schoolId: req.params.schoolId, actor: req.user._id }); res.json({ success: true, account: { id: result.account._id, school: result.account.school, accountName: result.account.accountName, accountNumberLast4: result.account.accountNumberLast4, verified: result.account.verified, active: result.account.active }, evidence: result.evidence }); } catch (error) { errorResponse(res, error); } };
exports.adminEduPayDuty = async (req, res) => { try { const permissions = [...new Set((Array.isArray(req.body.permissions) ? req.body.permissions : []).map(String))]; const allowed = new Set(["account.manage", "account.verify", "settlement.process"]); if (!permissions.length || permissions.some((permission) => !allowed.has(permission))) return res.status(400).json({ success: false, message: "Invalid EduPay duty permissions." }); const assignment = await EduPayDutyAssignment.findOneAndUpdate({ user: req.params.userId }, { $set: { permissions, active: req.body.active !== false, assignedBy: req.user._id } }, { upsert: true, new: true, runValidators: true }); await audit({ actor: req.user._id, action: "EDUPAY_DUTY_ASSIGNED", entityType: "EduPayDutyAssignment", entityId: assignment._id, metadata: { user: req.params.userId, permissions, active: assignment.active }, req }); res.json({ success: true, assignment }); } catch (error) { errorResponse(res, error); } };
exports.adminRevokeEduPayDuty = async (req, res) => { try { const assignment = await EduPayDutyAssignment.findOneAndUpdate({ user: req.params.userId }, { $set: { active: false } }, { new: true }); if (!assignment) return res.status(404).json({ success: false, message: "EduPay duty assignment not found." }); await audit({ actor: req.user._id, action: "EDUPAY_DUTY_REVOKED", entityType: "EduPayDutyAssignment", entityId: assignment._id, metadata: { user: req.params.userId }, req }); res.json({ success: true, assignment }); } catch (error) { errorResponse(res, error); } };
