...
exports.adminReadiness = async (req, res) => { try { const modelEntries = Object.entries(models); await EduPayDutyAssignment.init(); await Promise.all(modelEntries.filter(([, model]) => model?.init).map(([, model]) => model.init())); const indexes = {}; await Promise.all(modelEntries.filter(([, model]) => model?.collection?.listIndexes).map(async ([name, model]) => { indexes[name] = await model.collection.listIndexes().toArray(); })); const all = await EduPayDutyAssignment.find({}).sort({ user: 1, version: -1 }).lean(); const latest = new Map(); all.forEach((row) => { if (!latest.has(String(row.user))) latest.set(String(row.user), row); }); const current = [...latest.values()].filter((row) => row.active); const holders = (permission) => new Set(current.filter((row) => row.permissions.includes(permission)).map((row) => String(row.user))); const manageUsers = holders("account.manage"); const verifyUsers = holders("account.verify"); const processUsers = holders("settlement.process"); const viableDutySeparation = [...manageUsers].some((manager) => [...verifyUsers].some((verifier) => verifier !== manager && [...processUsers].some((processor) => processor !== manager && processor !== verifier))); const dutyCoverage = { manage: manageUsers.size, verify: verifyUsers.size, process: processUsers.size, viableDutySeparation, ready: viableDutySeparation }; res.json({ success: true, ready: dutyCoverage.ready, dutyCoverage, checkedAt: new Date(), indexes }); } catch (error) { errorResponse(res, error); } };
