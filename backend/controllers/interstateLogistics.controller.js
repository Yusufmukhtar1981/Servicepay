...
const assertRouteDirection = (route, body) => {
  ...
  if (!originState || !destinationState) throw new Error("Origin and destination states are required.");
  if (originState !== normalizedState(route.originState) || destinationState !== normalizedState(route.destinationState)) {
    throw new Error("The selected route does not match this shipment direction.");
  }
  if (body.sender?.state && normalizedState(body.sender.state) !== originState) throw new Error("Sender state must match the selected route origin.");
  if (body.receiver?.state && normalizedState(body.receiver.state) !== destinationState) throw new Error("Receiver state must match the selected route destination.");
};
const getRouteQuote = async (body, session = null) => {
  let routeQuery = LogisticsRoute.findOne(activeRouteFilter({ _id: body.routeId }));
...
exports.createShipment = async (req, res) => {
  ...
  try {
    ...
    const [shipment] = await Shipment.create([{ customerId: req.user._id, routeId: route._id, originBranchId: route.originBranchId, destinationBranchId: route.destinationBranchId, sender: b.sender, receiver: b.receiver, pickupMethod: b.pickupMethod, deliveryMethod: b.deliveryMethod, parcel: { ...b.parcel, dimensions: input.dimensions }, serviceType: b.serviceType, protection: !!b.protection, quote: { routeVersion: savedQuote.routeVersion, breakdown: quote.breakdown, total: quote.total, expectedDelivery: quote.expectedDelivery }, status: "AWAITING_PAYMENT" }], { session });
