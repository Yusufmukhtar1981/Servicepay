...
const populateDelivery = (deliveryId) => {
...
exports.getRiderDeliveries = async (
  req,
  res
) => {
  try {
...
exports.getRiderDeliveryDetails = async (
  req,
  res
) => {
  try {
...
exports.acceptRiderDelivery = async (
  req,
  res
) => {
  try {
    ...
    if (
      ...
      return res.status(403).json({
        success: false,
        message:
          "Only Delivery Riders can accept delivery jobs.",
      });
    }
    ...
    delivery.riderAcceptedAt = now;
    delivery.riderRejectedAt = null;
    ...
    rider.totalAcceptedDeliveries =
      Number(
        rider.totalAcceptedDeliveries || 0
      ) + 1;
    ...
    const updatedDelivery =
      await populateDelivery(delivery._id);
...
exports.rejectRiderDelivery = async (
  req,
  res
) => {
  try {
    ...
    if (
      ...
      return res.status(403).json({
        success: false,
        message:
          "Only Delivery Riders can reject delivery jobs.",
      });
    }
    ...
    try {
      ...
      if (
        ...
        return res.status(403).json({
          success: false,
          message:
            "Only Delivery Riders can update delivery jobs.",
        });
      }
