...
test("interstate quote is route-authoritative with full breakdown", () => {
  ...
  assert.deepEqual(quote.breakdown, { transportFee: 3000, sizeSurcharge: 0, expressSurcharge: 750, fragileItemSurcharge: 0, pickupFee: 300, deliveryFee: 600, protectionFee: 250 });
  assert.equal(quote.total, 4900);
  assert.equal(quote.expectedDelivery, "1–2 business days");
});
test("interstate quote applies configured directional oversize pricing", () => {
  const quote = calculateInterstateQuote(
    { ...route, maximumDimensionCm: 100, oversizeSurcharge: 650 },
    { weightKg: 1, dimensions: { length: 120, width: 30, height: 20 }, declaredValue: 0, serviceType: "STANDARD", pickupMethod: "BRANCH_DROP_OFF", deliveryMethod: "BRANCH_COLLECTION" },
  );
  assert.equal(quote.breakdown.sizeSurcharge, 650);
  assert.equal(quote.total, 2750);
  const reverified = calculateInterstateQuote(
    { ...route, maximumDimensionCm: 100, oversizeSurcharge: 650 },
    { weightKg: 1, dimensions: { length: 120, width: 30, height: 20 }, declaredValue: 0, serviceType: "STANDARD", pickupMethod: "BRANCH_DROP_OFF", deliveryMethod: "BRANCH_COLLECTION" },
  );
  assert.equal(reverified.total, quote.total);
  assert.throws(() => calculateInterstateQuote(
    { ...route, maximumDimensionCm: 100, oversizeSurcharge: 650 },
    { weightKg: 1, dimensions: { length: 90, diagonal: 120 }, declaredValue: 0, serviceType: "STANDARD", pickupMethod: "BRANCH_DROP_OFF", deliveryMethod: "BRANCH_COLLECTION" },
  ), /only length/);
});
