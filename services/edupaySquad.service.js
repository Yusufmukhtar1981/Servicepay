...
const Settlement = require("../backend/models/edupaySettlement.model");
const Account = require("../backend/models/edupaySettlementAccount.model");
const Evidence = require("../backend/models/edupayPayoutEvidence.model");
const Transaction = require("../backend/models/transaction.model");
const Plan = require("../backend/models/edupayPlan.model");
const { EduPayRepayment } = require("../backend/models/edupayRepayment.model");
const { createEduLedger, availableSavings, getSettings, audit, reverseSettlement, reference, round } = require("../backend/services/edupay.service");
const Commission = require("../backend/models/edupayCommission.model");
...
async function finalizeEvidence(evidenceId, actor, req, session) {
  ...
  if (settlement.schoolCommissionAmount > 0) await Commission.create([{ settlement: settlement._id, school: settlement.school, amount: settlement.schoolCommissionAmount, direction: "RECEIVABLE", reference: `${settlement.reference}-COMMISSION`, createdBy: actor }], { session });
  ...
  await Settlement.updateOne({ _id: settlement._id, status: "PROCESSING" }, { $set: { status: "SETTLED", providerReference: evidence.providerReference, provider: "SQUAD", confirmedBy: actor, confirmedAt: new Date() } }, { session });
...
async function recordProviderEvidence({ settlement, payload, source, raw, actor, req }) {
  const data = normalized(payload); if (!data.reference || !["SUCCESSFUL", "REVERSED"].includes(data.status)) throw fail("Provider evidence is incomplete or not final.", 409);
  ...
  const session = await mongoose.startSession(); let output; let reversalTransaction;
  try { await session.withTransaction(async () => {
    const current = await Settlement.findById(settlement._id).session(session); if (!current || !["PROCESSING", "SETTLED"].includes(current.status)) throw fail("Settlement is not awaiting provider evidence.", 409);
  ...
  if (data.status === "REVERSED" && reversalTransaction) await reverseSettlement({ settlementId: settlement._id, actor, transactionId: reversalTransaction._id, providerReference: reversalTransaction.reference, idempotencyKey: `squad-reversal-${eventDigest}`, req });
