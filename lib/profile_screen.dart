...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  bool biometricTransactionsEnabled = false;
  bool isUpdatingBiometrics = false;
  String biometricStatus = 'Checking device support...';
  ...
  Future<void> _loadBiometricState() async {
    final BiometricAuthService service = BiometricAuthService.instance;
    ...
    final bool loginEnabled = await service.hasLoginEnrollment() &&
        await service.loginOwner() == userId;
    final bool transactionsEnabled =
        userId.isNotEmpty && await service.hasTransactionEnrollment(userId);
    if (!availability.supported && (loginEnabled || transactionsEnabled)) {
      await service.clearAll();
    }
    if (!mounted) return;
    setState(() {
      biometricSupported = availability.supported;
      biometricStatus = availability.label;
      biometricLoginEnabled = availability.supported && loginEnabled;
      biometricTransactionsEnabled =
          availability.supported && transactionsEnabled;
    });
  }

  Future<void> _toggleBiometricLogin(bool enabled) async {
    if (isUpdatingBiometrics) return;
    setState(() => isUpdatingBiometrics = true);
    final BiometricAuthService service = BiometricAuthService.instance;
    try {
      if (!enabled) {
        await service.clearLoginEnrollment();
      } else {
        final SharedPreferences prefs = await SharedPreferences.getInstance();
        final String userId = (prefs.getString('user_id') ?? '').trim();
        final String token = (prefs.getString('auth_token') ?? '').trim();
        final String role =
            (prefs.getString('user_role') ?? '').trim().toUpperCase();
        if (userId.isEmpty || token.isEmpty || role != 'CUSTOMER') {
          throw StateError('Sign in as a customer before enabling biometrics.');
        }
        final BiometricAuthenticationResult result = await service.authenticate(
          reason: 'Enable biometric login for this ServicePay account',
        );
        if (result != BiometricAuthenticationResult.success) {
          throw StateError('Biometric setup was not completed.');
        }
      ...
      if (mounted) setState(() => isUpdatingBiometrics = false);
    }
  }

  Future<void> _toggleTransactionBiometrics(bool enabled) async {
    if (isUpdatingBiometrics) return;
    setState(() => isUpdatingBiometrics = true);
    final BiometricAuthService service = BiometricAuthService.instance;
    try {
      if (!enabled) {
        ...
        if (user['transactionPinSet'] != true) {
          throw StateError(
              'Create a transaction PIN before enabling biometrics.');
        }
        ...
        final http.Response response = await http
            .post(
              Uri.parse('$baseUrl/transaction-pin/verify'),
        ...
        if (response.statusCode < 200 || response.statusCode >= 300) {
          final dynamic data = _decodeResponse(response.body);
          throw StateError(
            _extractMessage(data, fallback: 'Incorrect transaction PIN.'),
          );
        }
        final BiometricAuthenticationResult result = await service.authenticate(
          reason: 'Enable biometrics for ServicePay transactions',
        );
        if (result != BiometricAuthenticationResult.success) {
          throw StateError('Biometric setup was not completed.');
        }
        await service.enrollTransaction(userId: userId, pin: pin);
      }
      await _loadBiometricState();
      _showMessage(
        enabled
            ? 'Transaction biometrics enabled.'
            : 'Transaction biometrics disabled.',
        isError: false,
      );
    } catch (error) {
      _showMessage(
    ...
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        if (response.statusCode == 401 || response.statusCode == 403) {
          await BiometricAuthService.instance.clearAll();
        }
        throw Exception(
          _extractMessage(
            decoded,
  ...
  Future<void> openChangeTransactionPin() async {
    final bool? updated = await Navigator.of(context).push<bool>(
      MaterialPageRoute<bool>(
        builder: (_) => const ChangeTransactionPinScreen(),
      ),
    );
    if (updated == true) {
      await _loadBiometricState();
      await loadProfile(showRefreshLoader: true);
    }
  }
  ...
  Future<void> logout() async {
    final bool? keepBiometrics = await showDialog<bool>(
      context: context,
      builder: (context) {
        bool keep = biometricLoginEnabled;
              ...
              children: <Widget>[
                ...
                if (biometricLoginEnabled)
                  CheckboxListTile(
                    ...
                    title: const Text('Keep biometric login on this device'),
                    value: keep,
                    onChanged: (value) {
                      setDialogState(() => keep = value == true);
                    },
                  ),
              ],
            ),
                        ...
                        children: <Widget>[
                          ...
                          _BiometricSwitchTile(
                            ...
                            subtitle: biometricSupported
                                ? 'Approve with biometrics or enter your PIN'
                                : biometricStatus,
                            value: biometricTransactionsEnabled,
                          ...
                          _ProfileActionTile(
                            ...
                            onTap: user['transactionPinSet'] == true
                                ? openChangeTransactionPin
                                : null,
                          ),
                          _ProfileActionTile(
                            icon: Icons.lock_reset_rounded,
                            title: 'Reset Transaction PIN',
                            subtitle: 'Reset with your account password',
                            showDivider: false,
                            onTap: user['transactionPinSet'] == true
                                ? openTransactionPin
                                : null,
...
class _BiometricSwitchTile extends StatelessWidget {
