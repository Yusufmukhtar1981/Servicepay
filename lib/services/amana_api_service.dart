...
class AmanaApiService {
  ...
  Future<Map<String, String>> _headers({bool json = true}) async {
    ...
    if (token == null) throw const AmanaApiException('Please log in to continue.', 401);
  ...
  Future<Map<String, dynamic>> get(String path, {Map<String, String>? query}) async =>
      _decode(await http.get(_uri(path, query), headers: await _headers()).timeout(const Duration(seconds: 45)));

  Future<Map<String, dynamic>> post(String path, {Map<String, dynamic> body = const <String, dynamic>{}}) async =>
      _decode(await http.post(_uri(path), headers: await _headers(), body: jsonEncode(body)).timeout(const Duration(seconds: 60)));

  Future<Map<String, dynamic>> patch(String path, {Map<String, dynamic> body = const <String, dynamic>{}}) async =>
      _decode(await http.patch(_uri(path), headers: await _headers(), body: jsonEncode(body)).timeout(const Duration(seconds: 60)));
  ...
  Map<String, dynamic> _decode(http.Response response) {
    dynamic decoded;
    try { decoded = jsonDecode(response.body); } catch (_) { decoded = <String, dynamic>{}; }
    final Map<String, dynamic> body = decoded is Map
        ? Map<String, dynamic>.from(decoded)
        : <String, dynamic>{};
    if (response.statusCode < 200 || response.statusCode >= 300) {
      throw AmanaApiException(
        body['message']?.toString() ?? body['error']?.toString() ?? 'Amana request failed.',
        response.statusCode,
      );
    }
    return body;
