...
class _BusinessPartnerOfficersScreenState
  ...
  bool get _canManage {
    return businessPartnerHasPermission(widget.profile, 'OFFICER_MANAGEMENT');
  }
    ...
    try {
      ...
      if (grouped is Map) {
        for (final String key in <String>['solar', 'phone']) {
          ...
          if (values is List) {
            rows.addAll(values.whereType<Map>().map((Map value) =>
                <String, dynamic>{...Map<String, dynamic>.from(value), 'type':
                    (value['type'] ?? key).toString().toUpperCase()}));
          }
        }
      } else if (grouped is List) {
        rows.addAll(grouped.whereType<Map>().map(
            (Map value) => Map<String, dynamic>.from(value)));
      }
      if (!mounted) return;
      setState(() { _officers = rows; _loading = false; });
  ...
  List<Map<String, dynamic>> get _visible => _filter == 'ALL'
      ? _officers
      : _officers.where((Map<String, dynamic> row) =>
          _text(row['type'], '').toUpperCase() == _filter).toList();
      ...
      child: SingleChildScrollView(
        ...
        child: ConstrainedBox(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: <
            ...
            else if (_error.isNotEmpty) _errorView()
            else if (_visible.isEmpty) _emptyView()
            else _list(),
          ]),
        ),
      ),
  ...
  Widget _header() => LayoutBuilder(
        builder: (BuildContext context, BoxConstraints constraints) {
          ...
          final Widget action = _canManage && _allowedTypes.isNotEmpty
              ? FilledButton.icon(
            ...
            Row(children: <Widget>[
              const Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                    children: <Widget>[
                  Text('My Officers', style: TextStyle(color: _officerInk,
  ...
  Widget _filterButton(String value, String label) => ChoiceChip(
        ...
        labelStyle: TextStyle(color: _filter == value ? _officerGreen : _officerMuted,
            fontSize: 12, fontWeight: FontWeight.w800),
        side: const BorderSide(color: _officerLine),
        backgroundColor: Colors.white,
      );

  Widget _list() => LayoutBuilder(builder: (BuildContext context, BoxConstraints box) {
        final bool wide = box.maxWidth >= 760;
        final List<Widget> cards = _visible.map(_card).toList();
        return wide
            ? Wrap(spacing: 13, runSpacing: 13,
                children: cards.map((Widget card) => SizedBox(
                    width: (box.maxWidth - 13) / 2, child: card)).toList())
  ...
  Widget _card(Map<String, dynamic> row) {
    ...
    final Map<String, dynamic> metrics = row['metrics'] is Map
        ? Map<String, dynamic>.from(row['metrics'] as Map) : <String, dynamic>{};
    final bool active = _text(row['status'], 'ACTIVE').toUpperCase() == 'ACTIVE';
    return Card(
      key: Key('officer-${_text(row['id'], _text(row['officerCode'], 'record'))}'),
      ...
      child: InkWell(
        ...
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <
              Widget>[
            Row(children: <Widget>[
              CircleAvatar(backgroundColor: const Color(0xFFE4F5EC),
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                  children: <Widget>[
                Text(_text(row['fullName'], 'Officer'), style: const TextStyle(
                    color: _officerInk, fontSize: 15, fontWeight: FontWeight.w900)),
                const SizedBox(height: 3),
                Text('${type == 'SOLAR' ? 'Solar' : 'Phone'} field officer'
                    '${row['officerCode'] == null ? '' : '  ·  ${_text(row['officerCode'])}'}',
                    style: const TextStyle(color: _officerMuted, fontSize: 11)),
              ])),
              _status(active),
            ]),
            ...
            Wrap(spacing: 18, runSpacing: 8, children: <Widget>[
              ...
              _detail(Icons.place_outlined, _text(row['lga'], _text(row['state']))),
            ]),
  ...
  Widget _detail(IconData icon, String value) => Row(mainAxisSize: MainAxisSize.min,
      children: <Widget>[Icon(icon, size: 15, color: _officerMuted),
        const SizedBox(width: 5), Text(value, style: const TextStyle(
            color: _officerMuted, fontSize: 12, fontWeight: FontWeight.w600))]);

  Widget _metric(String label, dynamic value) => Column(
      crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
    Text(_number(value), style: const TextStyle(color: _officerInk,
  ...
  String _initials(String name) {
    final List<String> words = name.split(' ').where((String x) => x.isNotEmpty).toList();
    return words.take(2).map((String x) => x[0].toUpperCase()).join();
  }
  Widget _status(bool active) => Container(
      padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 5),
      decoration: BoxDecoration(color: active ? const Color(0xFFE4F5EC) : const Color(0xFFFFEEE9),
          borderRadius: BorderRadius.circular(20)),
      child: Text(active ? 'Active' : 'Suspended', style: TextStyle(
          color: active ? _officerGreen : const Color(0xFFB04C38),
          fontSize: 10, fontWeight: FontWeight.w900)));

  Widget _loadingView() => Column(children: <Widget>[
    for (int i = 0; i < 3; i++) Container(height: 150, margin: const EdgeInsets.only(bottom: 13),
  ...
  Widget _errorView() => _message(
      ...
      'Try again',
      _load);
  Widget _emptyView() => _message(Icons.groups_outlined, 'No officers yet',
      'Create your first field officer to start routing work.', 'Create officer',
      _canManage ? () => _openForm() : null);
  Widget _message(IconData icon, String title, String body, String action,
      VoidCallback? callback) => Container(padding: const EdgeInsets.all(28),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(17),
          border: Border.all(color: _officerLine)),
      child: Column(children: <Widget>[Icon(icon, color: _officerGreen, size: 34),
        const SizedBox(height: 10), Text(title, style: const TextStyle(color: _officerInk,
            fontWeight: FontWeight.w900, fontSize: 16)), const SizedBox(height: 5),
        Text(body, textAlign: TextAlign.center, style: const TextStyle(color: _officerMuted)),
        if (callback != null) ...<Widget>[const SizedBox(height: 16),
  ...
  Future<void> _openDetail(Map<String, dynamic> row) async {
    ...
    if (id.isNotEmpty) {
      try {
        ...
        detail.addAll(payload is Map
            ? Map<String, dynamic>.from(payload)
            : response);
      } catch (_) {}
    }
    if (!mounted) return;
    showDialog<void>(context: context, builder: (_) => _detailDialog(detail, type, id));
  }
  ...
  Widget _detailDialog(Map<String, dynamic> row, String type, String id) {
    ...
    return AlertDialog(
      ...
      content: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start,
        ...
        _dialogLine('Phone', _text(row['phone'])), _dialogLine('Email', _text(row['email'])),
        _dialogLine('Territory', '${_text(row['lga'])}, ${_text(row['state'])}'),
        _dialogLine('Address', _text(row['address'])),
      ])),
      actions: <Widget>[
        if (_canManage) TextButton(key: const Key('edit-officer'),
            onPressed: () async {
        ...
        if (_canManage) TextButton(key: const Key('reset-access'),
            onPressed: () async {
              Navigator.pop(context);
              await _afterDialogCloses();
        ...
        if (_canManage) TextButton(key: const Key('toggle-officer-status'),
            onPressed: () async {
              Navigator.pop(context);
              await _afterDialogCloses();
  ...
  Widget _dialogLine(String label, String value) => Padding(
      padding: const EdgeInsets.only(bottom: 8), child: Text('$label  $value',
          style: const TextStyle(color: _officerInk, fontSize: 13)));

  Future<void> _openForm({Map<String, dynamic>? existing}) async {
    final bool editing = existing != null;
    final Map<String, TextEditingController> c = <String, TextEditingController>{
      for (final String key in <String>['fullName','phone','email','state','lga','address','password'])
    ...
    String type = _text(existing?['type'],
            allowedTypes.isEmpty ? 'SOLAR' : allowedTypes.first)
        .toUpperCase();
    final GlobalKey<FormState> formKey = GlobalKey<FormState>();
    await showDialog<void>(context: context, builder: (BuildContext dialogContext) {
      return StatefulBuilder(builder: (BuildContext context, StateSetter setDialog) =>
          AlertDialog(
            title: Text(editing ? 'Edit officer' : 'Create officer'),
            content: SizedBox(width: 520, child: Form(key: formKey, child: SingleChildScrollView(
              child: Wrap(spacing: 12, runSpacing: 2, children: <Widget>[
                if (!editing) DropdownButtonFormField<String>(value: type, decoration: const InputDecoration(labelText: 'Service'),
                  items: allowedTypes.map((String value) => DropdownMenuItem<String>(
                    value: value,
                    child: Text(value == 'SOLAR' ? 'Solar' : 'Phone financing'),
                  )).toList(),
                  onChanged: (String? value) => setDialog(() => type = value ?? type)),
                for (final String key in <String>['fullName','phone','email','state','lga','address'])
                  SizedBox(width: 245, child: TextFormField(controller: c[key], decoration: InputDecoration(labelText: _label(key)),
                    validator: (String? value) => value == null || value.trim().isEmpty ? 'Required' : null)),
                if (!editing) SizedBox(width: 245, child: TextFormField(controller: c['password'], obscureText: true,
                  decoration: const InputDecoration(labelText: 'Temporary password'),
                  validator: (String? value) => (value ?? '').length < 8 ? 'Use at least 8 characters' : null)),
              ]),
            ))),
            actions: <Widget>[
              TextButton(onPressed: () => Navigator.pop(dialogContext), child: const Text('Cancel')),
              FilledButton(key: const Key('save-officer'), onPressed: () async {
                if (!(formKey.currentState?.validate() ?? false)) return;
                try {
                  if (editing) {
                    await widget.api.updateOfficer(type: type, id: _text(existing['id']),
                        fields: <String, dynamic>{for (final String key in <String>['fullName','phone','email','state','lga','address']) key: c[key]!.text.trim()});
                  } else {
                    await widget.api.createOfficer(type: type, fullName: c['fullName']!.text.trim(), phone: c['phone']!.text.trim(),
                      email: c['email']!.text.trim(), password: c['password']!.text, state: c['state']!.text.trim(),
                      lga: c['lga']!.text.trim(), address: c['address']!.text.trim());
                  }
  ...
  String _label(String value) => value == 'fullName' ? 'Full name' :
      value[0].toUpperCase() + value.substring(1);

  Future<void> _confirmStatus(String type, String id, bool active) async {
    final bool? confirmed = await showDialog<bool>(context: context, builder: (_) =>
        AlertDialog(title: Text(active ? 'Suspend officer?' : 'Reactivate officer?'),
          content: Text(active ? 'This officer will no longer receive new assignments.' :
              'This officer can receive assignments again.'),
          actions: <Widget>[TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancel')),
            FilledButton(key: const Key('confirm-status'), onPressed: () => Navigator.pop(context, true),
                child: Text(active ? 'Suspend' : 'Reactivate'))]));
    if (confirmed == true) {
      try {
  ...
  Future<void> _resetDialog(String type, String id) async {
    ...
    final bool? confirmed = await showDialog<bool>(context: context, builder: (_) =>
        AlertDialog(title: const Text('Reset access'),
          content: TextField(controller: password, obscureText: true,
              decoration: const InputDecoration(labelText: 'New password')),
          actions: <Widget>[TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancel')),
            FilledButton(key: const Key('confirm-reset'), onPressed: () => Navigator.pop(context, true),
                child: const Text('Reset password'))]));
    if (confirmed == true && password.text.length >= 8) {
      try {
        await widget.api.resetOfficerAccess(
            type: type, id: id, password: password.text);
        if (mounted) {
